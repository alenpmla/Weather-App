...
class _WeatherAppMainScreenState extends State<WeatherHomeScreen> {
    ...
    return Scaffold(
      ...
      body: SmartRefresher(
        ...
        child: SingleChildScrollView(
          child: BlocConsumer<WeatherDetailsBloc, WeatherDetailsState>(
            listener: (context, state) {
              _refreshController.refreshCompleted();
            },
            builder: (context, state) {
              if (state is WeatherDetailsSuccess) {
                return _successView(state.weatherDetails, state.isCelsius,state.selectedDay);
  ...
  Column _successView(WeatherDetails weatherDetails, bool isCelsius, int selectedDay) {
    return Column(
      children: [
        Padding(
          padding: const EdgeInsets.only(left: 16, right: 16),
          child: WeatherDetailsWidget(
              weatherDetails: weatherDetails, isCelsius: isCelsius,
              selectedDay:selectedDay),
        ),
        const SizedBox(
          height: 16,
        ),
        UpComingWeatherList(weatherDetails, isCelsius),
      ],
    );
  }
}
